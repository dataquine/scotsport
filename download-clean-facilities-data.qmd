---
title: "Data cleaning - Facilities"
...
> ### **Terms and Conditions from** Spatial Hub Scotland
>
> The dataset is provided under [Open Government Licence (OGL)](https://www.nationalarchives.gov.uk/doc/open-government-licence/version/3/) for download and use. You are free to copy, publish, distribute and transmit the information as long as you acknowledge the source as coming from Improvement Service under OGL.
>
> Please note: while the Improvement Service shall make reasonable efforts ("Best Endeavours") to collect and provide accurate and up-to-date data, the Improvement Service does not guarantee the accuracy, completeness, or reliability of the Data provided. By accessing the data you acknowledge that the data may contain errors, omissions, or inconsistencies, and use the data at your own risk.
>
...
>
> Where there were missing coordinates for some facilities, manual lookups of appropriate coordinates have been made. Also, a judgement has been made to delete some provided facilities where they appear to no longer exist (for whatever reason).
>
> Provided layers are:
>
...
>
> Please note that the information provided is provided by third parties and therefore we cannot guarantee its accuracy, but it is the most up to date information we hold.
>
> As part of our work to keep this data up to date, we would kindly request that if you identify any issues, you share this with sportscotland at facilities\@sportscotland.org.uk
...
```{r}
#| label: data-source-facility-info
url_data_source <- "https://data.spatialhub.scot/dataset/sports_facilities-unknown"

# Data source acknowledge
# The dataset is provided under Open Government Licence (OGL) for download and
# use. You are free to copy, publish, distribute and transmit the information as
# long as you acknowledge the source as coming from Improvement Service under
# OGL.
...
# Spatial Hub is merely hosting the data for Sport Scotland, caveat
data_caveat_spatial_hub <- "Please note that the information provided is provided by third parties and therefore we cannot guarantee its accuracy, but it is the most up to date information we hold."
# View(df_facility_type_info)
df_facility_type_info
```

```{r}
#| label: data-source-dowload
...
```{r}
#| label: build-downloaded-file-list
...
df_scotland_sports_facilities_raw <- map(csv_downloaded_files, ~ read_csv(.x))
...
df_scotland_sports_facilities_combined <-
  list_rbind(df_scotland_sports_facilities_raw, names_to = "slug")
rm(df_scotland_sports_facilities_raw)
```

```{r}
#| label: clean-facilities-data

# Fix inconsistencies in column
df_scotland_sports_facilities_clean <- df_scotland_sports_facilities_combined |>
  ...
  #
  # mutate(town_is_clean = town != toupper(town), .after = town) |>
  # Think we should rename so that by is after rather than leading for
  # alphabetical sorting
  # mutate(town_is_clean = ifelse(stringr::str_starts(town, "By "),
  #   FALSE, town_is_clean
  # )) |>
  ...
  # fid = uniqie number within one facility type
  rename(id = FID, date_updated = sh_date_uploaded, name = site_name) |>
  relocate(slug, .after = id)

# rm(df_scotland_sports_facilities_combined)
# View(df_scotland_sports_facilities_clean)

# Standardise Towns
df_scotland_sports_facilities_clean <- df_scotland_sports_facilities_clean |>
  mutate(
    ...
    town_clean = stringr::str_replace(town_clean, "Berwick-Upon-Tweed", "Berwick-upon-Tweed"),
    town_clean = stringr::str_replace(town_clean, "Grantown On Spey", "Grantown-on-Spey"),
    town_clean = stringr::str_replace(town_clean, "Grantown-On-Spey", "Grantown-on-Spey"),
    town_clean = stringr::str_replace(town_clean, "Newport-On-Tay", "Newport-on-Tay"),
    town_clean = stringr::str_replace(town_clean, "Star, Markinch", "Star of Markinch"),
    town_clean = stringr::str_replace(town_clean, "Cambeltown \\(Near\\)", "Campbeltown (Near)"),
    ...
    town_clean = stringr::str_replace(town_clean, "Dumfries And Galloway", "Campbeltown (Near)"),

    # Dunbartonshire - these seem like for Clydebank High School, 2 Janetta Street, G81 3EJ
    town_clean = stringr::str_replace(town_clean, "Dunbartonshire", "Clydebank"),
    ...
    town_clean = stringr::str_replace(town_clean, "Jamieson Drive", "Stornoway"),

    # Side effect of Title case
    # pub_spfsh.2612 sports-halls, Kincardine O'Neil School, North Deeside Road, AB34 5AB
    town_clean = stringr::str_replace(town_clean, "Kincardine O'neil", "Kincardine O'Neil"),

    # pub_spfp.2195, pitches, Balmullo Primary School, Hayston Park, KY16 0DH
    town_clean = stringr::str_replace(town_clean, "Leuchars St Andrews", "St Andrews"),
    ...
    town_clean = stringr::str_replace(town_clean, "Shetland, Scotland", "Shetland"),

    # pub_spfp.3098, pitches,Smithton Primary School, 59 Smithton Park, IV2 7PD
    town_clean = stringr::str_replace(town_clean, "Smithton, Inverness", "Smithton"),

    # pub_spfp.2186, pitches,Arncroach Football Pitch,ARNCROACH,KY10 2RW
    town_clean = stringr::str_replace(town_clean, "St Monance \\(Near\\)", "Arncroach"),

    # pub_spfp.606, pitches, Strathmartine Primary School, DD3 0PH
    town_clean = stringr::str_replace(town_clean, "Strathmartine,", "Bridgefoot"),
...
df_scotland_sports_facilities_clean <- df_scotland_sports_facilities_clean |>
  ...
  mutate(
    ...
    # To save screen estate we'll label columns with Council so don't need it repeated
    la_name = stringr::str_remove(la_name, " Council"),
    
    facility_sub_type = ifelse (is.na(facility_sub_type), "Unspecified", facility_sub_type)
  )

long_lat <- df_scotland_sports_facilities_clean |>
...
df_scotland_sports_facilities_clean <-  bind_cols(df_scotland_sports_facilities_clean, long_lat)
  #cbind(df_scotland_sports_facilities_clean, long_lat) 
...
```{r}
#| label: write-clean-facilities-data
# Write out cleaned dataset
write_csv(
  df_scotland_sports_facilities_clean,
  here::here("data", params$filename_scotland_sports_facilities_csv)
)
```

```{r}
#| label: write-clean-population-data
# Write out cleaned dataset
# write_csv(df_scotland_sports_facilities_clean,
#           here::here("data", params$filename_scotland_sports_facilities_csv))
```

```{r}
df_scotland_sports_facilities_slim <- df_scotland_sports_facilities_clean |>
  select(
    -id, , -address, -town_orig, -eastings_x,
    -northing_y, -fid, -facid, -site_id
  )
```

```{r}
df_scotland_sports_facilities_slim |>
  select_if(is.character) |>
  ## map(~ dplyr::count(data.frame(x = .x), x)) #, sort = TRUE))
  map(~ dplyr::count(tibble(x = .x), x)) # , sort = TRUE))
```
